...
if ~isempty( g.notime )
    ...
    if max(g.notime(:)) > EEG.xmax
        g.notime(find(g.notime>EEG.xmax)) = EEG.xmax;
        disp('warning: Time/point range out of data limits. Corrected.');
    end
    if min(g.notime(:)) < EEG.xmin
        g.notime(find(g.notime<EEG.xmin)) = EEG.xmin;
        disp('warning: Time/point range out of data limits. Corrected.');
    end;
end;
if ~isempty(g.time)
...
% performing removal
% ------------------
if ~ischar(EEG.data) && (~isequal(g.channel,1:size(EEG.data,1)) || ~isequal(g.trial,1:size(EEG.data,3)))

function eeg_multiplotTFR(EEG,TF,cfg,what2plot)
% eeg_multiplotTFR(EEG,TF,cfg,what2plot)
% 
% create multiplotTFR using fieldtrip routine.
%
% inputs:
%       EEG:    an EEG structure
...
if ischar(EEG.data)
    EEG.data = NaN(EEG.nbchan,EEG.pnts,EEG.trials);% = pop_loadset(EEG.filename,EEG.filepath);
end
...
if isfield(cfg,'maskparameter')
    freq.(cfg.maskparameter) = permute(TF.(cfg.maskparameter),reorder);
end
...
try
    rm_frompath('eeglab')
    rm_frompath('spm')
    addpath('/home/chaumonm/fieldtrip');
    ft_defaults

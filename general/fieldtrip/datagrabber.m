...
% if only one acceptable value is entered (that is no | symbol separates
% distinct values of the field, the field will be ignored for conditions
% computation but kept in the evts structure so the values can be used
% subsequently.
...
[f] = flister(re,'exclude',g.exclude);
...
if strcmp(g.loadmode,'simple')
    ...
    for i_f = 1:numel(f)
        ...
        GRAB(1).allchans = {GRAB(1).allchans{:} GRAB(i_f).EEG.chanlocs.labels};
    end
 
    GRAB(1).allchans  = unique(GRAB(1).allchans);
    for i = 1:numel(GRAB)
        GRAB(i).allchans = GRAB(1).allchans;
    end
    g.range = makerange(GRAB,g.range);
end
...
if not(isempty(revts))
    ...
    if isfield(GRAB,'nconds') && GRAB(1).nconds == 1
        disp('You might well have to define conditions before proceeding...')
    end
end
...
for i = 1:numel(evts)
    evts(i).event = str2num(evts(i).event);
end
evtfs(toignore) = [];
...
if promptorder && numel(f) > 1
    ...
    disp(strjust(char(s),'right'));
    s = input('Give order of the factors: ','s');
    s = str2num(s);
...
if not(iscell(range.channels.idx)) && isinf(range.channels.idx(1))
    range.channels.idx = 1:numel(GRAB(1).allchans);
end
[range.channels.idx range.channels.chans] = chnb(range.channels.idx,GRAB(1).allchans);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

...
% pow = powspect(img)
% Compute the circularly averaged power spectrum of the fourier transform
% of img.


if isstr(img) % if it's a string, we assume file name
    Im = double(imread([img]));% read img
elseif isnumeric(img) % otherwise we assume image data directly
    Im = img;
end
if ndims(Im) == 3

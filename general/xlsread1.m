...
% [data, text, rawData, customOutput]=xlsread1(file,sheet,range,mode,customFun,Excel)
% same as XLSREAD except that we can input an activeX Excel object so we don't have to start it
% everytime.
%
...
%   $Revision: 1.23.4.21 $  $Date: 2007/06/14 05:10:43 $
%=============================================================================
...
custom = false;
if nargin >= 5 && not(isempty(customFun))
...
if nargin >= 6 && strcmp(class(Excel),'COM.excel_application')
    wasalreadystarted = 1;
else
    wasalreadystarted = 0;
    if strcmpi(mode,'basic') || ~ispc
...
%     aTemp = Excel.workbooks.Add(); aTemp.Close();
    ...
    %If workbooks has links, we should still not pop-up a dialog.
    askToUpdate = Excel.AskToUpdateLinks;  
    Excel.AskToUpdateLinks = 0;
    openalready = 0;
    if ~isempty(Excel.ActiveWorkbook)
        if isempty(strfind(file,Excel.ActiveWorkbook.Name))
    ...
    %make sure to go back to original setting.
    Excel.AskToUpdateLinks = askToUpdate;

function [top_int, bot_int,X] = regression_line_ci(alpha,beta,x,y,n_pts,x_min,x_max)
...
defifnotexist('x_min',min(x));
defifnotexist('x_max',max(x));
defifnotexist('n_pts',100);

X = linspace(x_min,x_max,n_pts);
Y = beta(1) + beta(2).*X;

SE_y_cond_x = sum((y - beta(1) - beta(2).*x).^2)/(N-2);
SSX = (N-1)*var(x);
SE_Y = SE_y_cond_x*((1/N + (mean(x)^2)/SSX) + (X.^2 - 2*mean(x)*X)/SSX);

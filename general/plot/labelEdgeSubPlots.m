function varargout=labelEdgeSubPlots(xl,yl,onlyBottom,alsoticklabels)
% labelEdgeSubPlots - label subplots only along edges
%
% function H=labelEdgeSubPlots(xl,yl,onlyBottom,alsoticklabels)    
%
% Purpose
...
%                be processsed. 
% alsoticklabels - also remove tick labels from x and y axes
%
% Outputs
...
% Max Chaumon - Nov 2012: don't touch colorbars, and alsoticklabels
    
if nargin <4, alsoticklabels=0;end
...
c(strmatch('cbar',get(c,'tag'))) = [];
c(strmatch('ColorBar',get(c,'tag'))) = [];
c(strmatch('legend',get(c,'tag'))) = [];
...
h.ylabels=[];
for i=1:numel(c)
    if pos(i,2) ~= min(pos(:,2));
         set(get(c(i),'xlabel'),'string','')
         if alsoticklabels
             set(c(i),'xticklabel',[]);
         end
    else
        h.xlabels=[h.xlabels,get(c(i),'xlabel')];
        set(h.xlabels(end),'string',xl)
    end
    if pos(i,1) ~= min(pos(:,1));
         set(get(c(i),'ylabel'),'string','')
         if alsoticklabels
             set(c(i),'yticklabel',[]);
         end
    else
        h.ylabels=[h.ylabels,get(c(i),'ylabel')];
